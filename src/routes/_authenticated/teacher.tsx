    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Create a class, then add students and take attendance.</p>
        </div>
        <div className="flex gap-2">
          <Link to="/teacher/leaves"><Button variant="outline"><Mail className="h-4 w-4 mr-1" />Leave requests</Button></Link>
          <Link to="/teacher/holidays"><Button variant="outline"><CalendarDays className="h-4 w-4 mr-1" />Holidays</Button></Link>
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild><Button><Plus className="h-4 w-4 mr-1" />New class</Button></DialogTrigger>
            <DialogContent>
              <DialogHeader><DialogTitle>Create class</DialogTitle></DialogHeader>
              <div className="space-y-3">
                <div><Label>Class name *</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="MSc CS 2026" /></div>
                <div><Label>Section</Label><Input value={form.section} onChange={(e) => setForm({ ...form, section: e.target.value })} placeholder="A" /></div>
                <div><Label>Subject</Label><Input value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} placeholder="Data Structures" /></div>
              </div>
...
function FacultyDashboard() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const cm = new Map((cls ?? []).map(c => [c.id, c.name] as const));
      setClassMap(cm);
      const classIds = [...cm.keys()];
      if (classIds.length === 0) { setStudents([]); setRows([]); setLoading(false); return; }

      const { data: studs } = await supabase.from("students")
        .select("id,name,email,class_id").in("class_id", classIds);
      const st = (studs ?? []) as Student[];
      setStudents(st);
      if (st.length === 0) { setRows([]); setLoading(false); return; }

      const sIds = st.map(s => s.id);
      const { data: att } = await supabase.from("attendance")
        .select("student_id,date,status").in("student_id", sIds);
      ...
      const t = todayISO();
      let tp = 0, ta = 0, th = 0, tm = 0;
      allRows.filter(r => r.date === t).forEach(r => {
        tm += 1;
        const v = Number(r.status);
        if (v === 1) tp += 1; else if (v === 0) ta += 1; else th += 1;
      });
  ...
  const stats = useMemo<StudentStat[]>(() => {
    ...
    const scoped = classFilter === "all" ? students : students.filter(s => s.class_id === classFilter);
    ...
    const per = new Map<string, { present: number; absent: number; half: number; sum: number; days: number }>();
    scoped.forEach(s => per.set(s.id, { present: 0, absent: 0, half: 0, sum: 0, days: 0 }));
    filtered.forEach(r => {
      const b = per.get(r.student_id); if (!b) return;
      const v = Number(r.status);
      b.sum += v; b.days += 1;
      if (v === 1) b.present += 1; else if (v === 0) b.absent += 1; else b.half += 1;
    });
    ...
    return scoped.map(s => {
      ...
      return {
        ...
        totalDays, present: b.present, absent: b.absent, half: b.half, percent,
      };
  ...
  const summary = useMemo(() => {
    ...
    const avg = total ? Math.round((stats.reduce((a, s) => a + s.percent, 0) / total) * 10) / 10 : 0;
    const markedDays = stats[0]?.totalDays ?? 0;
    return { total, good, warn, bad, avg, markedDays };
  }, [stats]);

  if (loading) {
    return <Card><CardContent className="py-8 text-center text-muted-foreground">Loading dashboard…</CardContent></Card>;
  }
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            {viewMode === "all" ? "All time" : `${MONTH_NAMES[selectedMonth - 1]} ${selectedYear}`} · based on {summary.markedDays} marked day{summary.markedDays === 1 ? "" : "s"}.
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <StatBox icon={<Users className="h-4 w-4" />} label="Total students" value={summary.total} />
        <StatBox icon={<Percent className="h-4 w-4" />} label="Average attendance" value={`${summary.avg}%`}
          tone={summary.avg >= 90 ? "good" : summary.avg >= 75 ? "warn" : "bad"} />
        <StatBox icon={<UserCheck className="h-4 w-4" />} label="Present today" value={today.present}
          hint={today.marked ? `${today.marked} marked` : "Not marked yet"} tone="good" />
        <StatBox icon={<UserX className="h-4 w-4" />} label="Absent today" value={today.absent}
          hint={today.half ? `${today.half} half-day` : undefined} tone={today.absent ? "bad" : undefined} />
      </div>

      <div className="grid gap-4 md:grid-cols-3">
        <CategoryCard tone="good" label="≥ 90% (safe)" count={summary.good} total={summary.total} />
        <CategoryCard tone="warn" label="75% – 89% (watch)" count={summary.warn} total={summary.total} />
        <CategoryCard tone="bad" label="Below 75% (critical)" count={summary.bad} total={summary.total} />
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              {atRisk.length === 0 ? "Everyone is above the 90% attendance criteria." : `${atRisk.length} student${atRisk.length===1?"":"s"} need${atRisk.length===1?"s":""} attention.`}
            </CardDescription>
          </div>
                  ...
                  {atRisk.map(s => {
                    ...
                    return (
                      <tr key={s.id} className="border-b last:border-0">
                        <td className="p-3">
                          <Link to="/teacher/student/$studentId" params={{ studentId: s.id }} className="font-medium hover:underline text-primary">
                            {s.name}
                          </Link>
...
function StatBox({ icon, label, value, hint, tone }: { icon: React.ReactNode; label: string; value: string | number; hint?: string; tone?: "good" | "warn" | "bad" }) {
...
function CategoryCard({ tone, label, count, total }: { tone: "good" | "warn" | "bad"; label: string; count: number; total: number }) {
  const pct = total ? Math.round((count / total) * 100) : 0;
  const bar = tone === "good" ? "bg-accent" : tone === "warn" ? "bg-yellow-500" : "bg-destructive";
  const text = tone === "good" ? "text-accent" : tone === "warn" ? "text-yellow-700" : "text-destructive";
  return (
    <Card>
      <CardContent className="p-5">
        <div className="flex items-center justify-between">
        ...
        <p className={`mt-1 text-3xl font-bold ${text}`}>{count}<span className="text-base text-muted-foreground font-normal"> / {total}</span></p>
