...
function ClassDetail() {
  ...
  const addStudent = async () => {
    ...
    const { error } = await supabase
      .from("students")
      .insert({ class_id: classId, ...form, email: form.email.trim().toLowerCase() });
        ...
        <Button
          ...
          onClick={async () => {
            if (
              klass &&
              confirm(
                `Are you sure you want to delete the class "${klass.name}"? This will delete all student rosters and attendance records in this class.`,
              )
            ) {
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        The student uses this email to sign up.
                      </p>
                    </div>
...
function TakeAttendance({ students }: { students: Student[] }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      (data ?? []).forEach((r: { student_id: string; status: number }) => {
        m[r.student_id] = Number(r.status);
      });
      setMarks(m);
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <CardDescription>
            Mark each student. 1 = Present, 0.5 = Half day, 0 = Absent.
          </CardDescription>
        </div>
        <div className="flex items-end gap-2">
          ...
          {!isHoliday ? (
            <Dialog open={holidayDialogOpen} onOpenChange={setHolidayDialogOpen}>
              <DialogTrigger asChild>
                <Button
                  variant="outline"
                  className="border-orange-200 hover:bg-orange-50 text-orange-700 hover:text-orange-900"
                >
                  Mark as Holiday
                </Button>
              </DialogTrigger>
              <DialogContent>
                ...
                <div className="space-y-3 py-2">
                  ...
                  <Input
                    ...
                    placeholder="e.g. Convocation, Festival"
                  />
                </div>
                <DialogFooter>
          ...
          <Button onClick={save} disabled={saving || isHoliday}>
            {saving ? "Saving…" : "Save"}
          </Button>
        </div>
      </CardHeader>
      {isHoliday && (
        <div className="mx-6 p-4 bg-orange-50 border border-orange-200 text-orange-800 rounded-xl flex items-center justify-between text-sm">
          <span className="flex items-center gap-2">
            ...
            <span>
              This date is marked as a <strong>Holiday ({holidayReason})</strong>. Attendance values
              are ignored for metrics on holidays.
            </span>
          </span>
          <Button
            size="sm"
            variant="ghost"
            className="text-orange-800 hover:text-red-700 hover:bg-orange-100"
  ...
  useEffect(() => {
    (async () => {
      ...
      const holidays = new Set<string>(
        (holidayRows ?? []).map((r: { holiday_date: string }) => r.holiday_date),
      );
      ...
      const { data: att } = await supabase
        ...
        .in("student_id", ids)
        .gte("date", ds[0]?.date ?? "")
        .lte("date", ds[ds.length - 1]?.date ?? "");
      const map = new Map<string, number>();
      (att ?? []).forEach((r: { student_id: string; date: string; status: number }) =>
        map.set(`${r.student_id}_${r.date}`, Number(r.status)),
      );
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-2 flex-wrap">
        <CardTitle>
          Matrix — {MONTH_NAMES[month - 1]} {year}
        </CardTitle>
            ...
            <table className="text-xs border-collapse">
              <thead>
                <tr>
                  ...
                  {dates.map((d) => (
                    <th key={d.date} className="p-1 border-b text-center min-w-[28px]">
                      {d.date.slice(-2)}
                    </th>
                  ))}
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.student.id}>
                    <td className="sticky left-0 bg-card p-2 border-b font-medium whitespace-nowrap">
                      {r.student.name}
                    </td>
                    {dates.map((d) => {
                      ...
                      const label =
                        v === "H" ? "H" : v === 1 ? "P" : v === 0 ? "A" : v === 0.5 ? "½" : "—";
                      return (
                        <td key={d.date} className={`p-1 border-b text-center ${cls}`}>
                          {label}
                        </td>
                      );
                    })}
